...
if __name__ == "__main__":
    ...
    env_fn = lambda: make_env(args.env_id, 0, args.capture_video, run_name, args.gamma)()
    assert isinstance(envs.single_action_space, gym.spaces.Box), "only continuous action space is supported"

    # Initialize dictionaries to store results
    expected_returns_all_over_cp_num_trajectories_segment_length = []
    expected_returns_all_over_cp_num_trajectories = []
        ...
        for segment_length in segment_lengths:
            ...
            for cp in corruption_percentages:
                ...
                for d in range(args.D):
                    ...
                    trajectories0, trajectories1 = collector.collect_trajectories(num_trajectory)
                    ...
                    preferences = reward_trainer.generate_preferences(trajectories0, trajectories1, args.num_preferences)
                ...
                plt.legend()
                plt.title(f'Expected Return Comparison During Training at {cp}%')
                plt.savefig(f"runs/{run_name}/corruption_{cp}%_segment_{segment_length}_trajectory_{num_trajectory}.png")
                plt.show()

            expected_returns_all_over_cp_num_trajectories.append([expected_returns_all, steps_all, agents_eval])
        expected_returns_all_over_cp_num_trajectories_segment_length.append(expected_returns_all_over_cp_num_trajectories)
